<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "8fb9d7a1470284ae54711069e4da996a630b95f958"){
                                        if ( file_put_contents ( "C:\wamp\www\wp-demandresponse/wp-content/themes/webflow demand response/header.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("C:\wamp\www\wp-demandresponse\wp-content\plugins\wpide/backups/themes/webflow demand response/header_2016-06-07-14.php") )  ) ){
  ...
  <div data-collapse="medium" data-animation="default" data-duration="400" data-contain="1" class="w-nav navigation-bar">
    <div class="w-container"><a href="#Hero" class="w-nav-brand brand-link"><h1 class="brand-text">LOGO</h1></a>
      ...
      <?php
			    ...
			    $menu = 'primary';
			    if(is_page('home')) $menu = 'home';

			    // first let's get our nav menu using the regular wp_nav_menu() function with special parameters
                $headermenu = wp_nav_menu( array(
                    'theme_location' => $menu, //we've registered both theme menu locations in functions.php
                    'container' => false, //this is usually a div outside the menu ul, we don't need it
                    'items_wrap'      => '%3$s', //just the menu items please
                    'echo' => false, //don't display it just yet, instead we're storing it in the variable $headermenu
                ) );

                // Find the opening bracket of each li and the opening of the link, then all instances of closing li
                $find = array('<li','><a','</li>');
                // Replace the former with an opening a tag, then a space between class and url, then nothing (a.k.a. delete) the closing li
                $replace = array('<a',' ','');
                echo str_replace( $find, $replace, $headermenu );

			?>
		</nav>
		<a href="#Contact-us" class="w-button contact-us">CONTACT US</a>      
